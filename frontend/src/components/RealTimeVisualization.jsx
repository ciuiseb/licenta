...
const RealTimeVisualization = ({ 
  ...
  // Track y-axis bounds to prevent jumping
  const [yBounds, setYBounds] = useState({ min: -1, max: 1 });
  
  // Point evaluation state
  ...
  // Update chart data when any solution data changes
  useEffect(() => {
    const datasets = [];
    let allYValues = [];
    
    // Add numerical solution (if available)
    ...
    // Update y-axis bounds if we have data
    ...
    // Throttle chart updates during training to prevent jitter
    ...
    if (isTraining && trainingData) {
      // Throttle updates during training (100ms delay)
      updateTimeoutRef.current = setTimeout(updateChart, 100);
    } else {
      // Immediate update when not training
      updateChart();
    }
  ...
  // Reset historical data when training resets
  useEffect(() => {
  ...
  // Chart options for real-time performance
  const chartOptions = useMemo(() => {
    // Get fixed axis ranges from domain or use defaults
            ...
            <div className="loss-metrics">
              <div className="metric-card">
                ...
                <div className="metric-trend">
                  ...
                  {lossData.total >= 0.001 && lossData.total < 0.01 && <span className="trend-ok">→ OK</span>}
            ...
            <div className="point-evaluation">
              ...
              {evalResult && (
                <div className="eval-result">
                  <strong>Result:</strong> y({evalResult.t}) = <span className="result-value">{evalResult.y.toFixed(6)}</span>
                </div>
              )}
